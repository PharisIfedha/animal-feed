...
if (isset($_POST['signup_btn'])) {
	...
	if (count($errors) === 0) {
		...
		$query = "INSERT INTO users SET username=?, email=?, password=?, profile_picture=?, created_at=?, Address=?, city=?, zip_code=?, country=?";
		$stmt = $conn->prepare($query);
		$stmt->bind_param('sssssssss', $username, $email, $password, $profile_picture, $created_at,$Address,$city,$zip_code,$country);
		$result = $stmt->execute();
		if ($result) {
...
if (isset($_POST['login_btn'])) {
	...
	if (empty($errors)) {
		...
		if (!empty($user)) { // if user was found
			...
			$_SESSION['error_msg'] = "You haven't Registered.";
		}
	}
}